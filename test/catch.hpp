...
namespace Catch {
    ...
    public:
        OcMethod( Classe cls, SEL sel ) : m_cls( cls ), m_sel( sel ) {}
    ...
    namespace Detail{
        ...
        inline std::string getAnnotation(   Classe cls,
    ...
    inline std::size_t registerTestMethods() {
        ...
        Classe* classes = (CATCH_UNSAFE_UNRETAINED Classe *)malloc( sizeof(Classe) * noClasses);
        objc_getClassList( classes, noClasses );

        for( int c = 0; c < noClasses; c++ ) {
            Classe cls = classes[c];
            {
                u_int count;
